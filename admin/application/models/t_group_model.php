 ...
 */
class T_group_model extends MY_Model
{
	/**
	 * @COLUMN_KEY		PRI
	 * @DATA_TYPE		int
	...
	*/
	...
	{
		return $this->db->get_where('t_group',array('gid' => $gid))->row();
	}
	...
	{
		return $this->db->count_all('t_group');
	}
	...
	{
		if(!isset($post['gid']) || empty($post['gid'])) return false;
		if(!isset($post['ctime']) || empty($post['ctime'])) return false;
		if(!isset($post['state']) || empty($post['state'])) return false;
	...
	*/
	...
	{
		return $this->db->delete('t_group',array('gid' => $gid));
	}
